...
public class ShoulderSubsystem extends SubsystemBase {
  ...
  public ShoulderSubsystem() {
    shoulderAngle1 = new WPI_VictorSPX(RobotMap.shoulderAngle1_CAN);
    shoulderAngle2 = new WPI_VictorSPX(RobotMap.shoulderAngle2_CAN);
    shoulderAngle1.setNeutralMode(NeutralMode.Brake);
    shoulderAngle2.setNeutralMode(NeutralMode.Brake);

    shoulderCoder = new CANCoder(RobotMap.shoulderCoder_CAN);
  ...
  public void changeShoulderAngle(double angleSpeed){
    ...
    if (angleSpeed > 0 && currentAngle >= RobotMap.shoulder_UpperLimit)
      angleSpeed = Math.min(0, angleSpeed);

    if (angleSpeed < 0 && currentAngle <= RobotMap.shoulder_LowerLimit)
